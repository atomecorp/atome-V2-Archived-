...
class Atome
  ...
  def atome_builder(object, properties)
    ...
    @atome << {atome_id: "#{object}_#{id_counter}_#{time}.#{location}"}
    ...
    ######### tests here ############
    @atome.each do |electron|
      p "the master type of electron is : " + electron.keys[0]
    end
    ######### tests here ############
    ...
    #atome format is : {"type"=>"box", "id"=>"box_0", "atome_id"=>"box_0_2020.01", "visible"=>"true"}
  ...
  def electron_builder(electron, value)
    ...
    if value.class == NilClass
      ...
      puts "important : here we must find what we expect to get!!!"
