...
class Atome
  ...
  def initialize(param = nil, add_atome_id = true, prop_type=nil)
    @prop_type=prop_type
    #above the first param is used to pass the value Has , the second param, add_atome_id is used to automaticaly add an id to the object is not specicifed to not do so , the third param (prop_type) is the only way get have a trace of the peroperty type of the new atome
    if param != nil
      @atome = param
    else
      @atome = {}
    ...
    if add_atome_id == true
      new_prop = {:value => object_id}
      # we create a new atome and send the add_atome_id to false to prevent recursive creation atome
      @atome[:atome_id] = Atome.new(new_prop, false, :atome_id)
    end
    #puts @atome
    #puts @atome[:type]
    #puts param
    @atome.each do |type, value|
      if type == :value
        puts "prop_type : #{prop_type}"
        puts value
        puts "-------"
    ...
    define_method(function) do |*values, &proc|
      ...
      if value
        ...
        if value.class == Hash
          ...
          @atome[function] = Atome.new(new_prop, false, function)
        end
        return self
      else
        get function
  ...
  def set prop = nil
    ...
    # we create a new atome and send the add_atome_id to false to prevent recursive creation atome
    @atome[prop_to_set] = Atome.new(new_prop, false,prop_to_set)
    return self
  end
  ...
  def add prop = nil
    if prop
      ...
      # we create a new atome and send the add_atome_id to false to prevent recursive creation atome
      # new_prop[:parent]=@atome[:atome_id][:value][:value]
      @atome[requested_prop] = Atome.new(new_prop, false, requested_prop)
    end
    return self
  end
  ...
  def get function = nil
    ...
    if function.to_s.end_with?("s")
      @atome[function] = Atome.new(collected_prop, false, function)
      return Atome.new(collected_prop, false,function)
