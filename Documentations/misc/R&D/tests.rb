...
#puts Atome.atome_methods
def parse code
  new_formated_code = []
  code = code.split("\n")
  code.each do |lines|
    lines.split(".").each_with_index do |parts|
      #if parts.split("")
      # puts parts
      param = parts.string_between_markers("(", ")")
      if (param)
        if param == ""
        else
          puts "---"
    ...
    #  delimiters = ['.', '(']
    #  portion = lines.split(Regexp.union(delimiters))
    #  if portion.length > 1
    #    # when split result in lore than 1 object we have to analysis the result
    #    prefix = portion.shift
    #    portion.each_with_index  do |val, index|
    #      if val.end_with?(")")
    #        value = val.chomp(")")
    #        puts value
    #      end
    #    end
    #    output = "prefix : #{prefix} , value : #{}"
    #    new_formated_code << "\n"
    #  else
    #    new_formated_code << lines + "\n"
    #  end
    #
  end
  #puts "new_formated_code : \n#{new_formated_code.join("")}"
end
...
tata("salut")
bubu = <<StrDelim
a = Atome.new([{:border => {:color => {color: :purple, x: 900}}}, {color: :pink}, {name: :my_nice_object}])
toto = (a.name)
toto="poimlk"
toto.split("m")
toto.gsub("i","mm")
StrDelim
a.color(:mauve)
a = Atome.new([{:border => {:color => {color: :red, x: 200}}}, {color: :pink}, {name: :my_nice_object}])
toto = (a.name)
toto="poimlk"
toto.split("m")
toto.gsub("i","mm")
toto.reverse
a.y = 200
a.border.color(:red).id.x(440)
a.shadow().border.color(:red).id(:toto).x(440)
a.id().label.x = (230)
a.shadow = (84)
a.z = 20
StrDeli
...
#  end
#end
#puts "blahblah.blahSTART(lkjlk).foofoo.foo) ENDwowowowowo".string_between_markers("(", ")")
